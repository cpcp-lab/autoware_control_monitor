...
def main():
    ...
    parser.add_argument('data_dir', nargs='?', default='1',
                        help='Directory containing control_log.csv and marker_log.csv (default: 1)')
    ...
    for i, (idx, row) in enumerate(m_subset.iterrows()):
        ...
        for j, r1 in mdf.loc[idx:idx_end].iterrows():
            ...
            # Annotation: curvature within spatial tolerance
            ann1 = (abs(kappa) * eps <= 1)
            # Annotation: waypoint reachability bound
            ann2 = (kappa * abs( (wx1**2 + wy1**2 - eps**2) / 2 - wy1 ) < eps)

            # Feasibility: waypoint is ahead and speed bounds are valid
            feas1 = (wx1 > 0 and 0 <= vl and vl < vh)
            feas2 = (aa*th <= vh - vl and bb*th <= vh - vl)

            feas = (ann1 and ann2 and feas1 and feas2)

            # Go condition: acceleration is within bounds and speed stays non-negative
            go1 = (-bb <= acc and acc <= aa and vel + acc*th >= 0)

            # Upper speed bound: vehicle does not exceed vh
            go_h1 = (vel <= vh and vel + acc*th <= vh)
            go_h2 = (1 + abs(kappa)*eps)**2 * (vel*th + (acc/2)*th**2 + ((vel + acc*th)**2 - vh**2) / (2*bb)) \
                    + eps <= max(abs(wx1),abs(wy1))
            go_h = (go_h1 or go_h2)

            #print('%d: %f v. %f + %f *th' %(j-idx, vl, vel, acc))

            # Lower speed bound: vehicle does not drop below vl
